...
struct ChattingStore {
    ...
    struct State: Equatable {
        ...
        var list: [ChatModel] = []
        var chattingState: NewChattingState = .waiting
    ...
    enum Action: Equatable {
        ...
        case updateChattingState(NewChattingState)
    ...
    var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case .appendRandomList:
                ...
                return .none
            case .updateChattingState(let chatState):
                state.chattingState = chatState
                return .none
            case .updateKeyboardOption(let option):
                state.keyboardOption = option
                return .none
            case .updateChatList(let model):
                state.list = model
                return .none
            }
        }
