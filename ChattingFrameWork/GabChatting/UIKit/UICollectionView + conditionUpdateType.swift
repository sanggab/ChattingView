...
extension ChattingCollectionView {
    func onAppearAction(_ uiView: UICollectionView, context: Context) {
        ...
        if size != .zero && contentSize != .zero && self.updateState == .onAppear {
...
// MARK: - textInput
extension ChattingCollectionView {
    func textInputAction(_ uiView: UICollectionView, context: Context) {
        print("\(#function) previousInputHeight: \(previousInputHeight)")
        print("\(#function) inputHeight: \(inputHeight)")
        print("\(#function) previousKeyboardHeight: \(previousKeyboardHeight)")
        print("\(#function) keyboardOption.size.height: \(keyboardOption.size.height)")
    ...
    func isConditionWithDifferenceKeyboardHeight(_ uiView: UICollectionView) {
        ...
        if self.previousKeyboardHeight != self.keyboardOption.size.height && self.previousKeyboardHeight != .zero {
    ...
    func isConditionWithDifferenceInputHeight(_ uiView: UICollectionView) {
        ...
        if self.previousInputHeight != self.inputHeight && self.previousInputHeight != .zero {
            print("move offset")
            let differenceHeight: CGFloat = self.inputHeight - self.previousInputHeight
            print("\(#function) differenceHeight: \(differenceHeight)")
            /// 맨 하단일 경우, 애니메이션 다르게 해주기
            /// 맨 상단일 경우, 안밀어주기
            self.executeAnimator(uiView, offsetY: uiView.contentOffset.y + differenceHeight)
        }
